 ...
 const API_URL=process.env.API_URL
  ...
  /*useEffect(() => {
    const fetchClient = async () => {
      try {
        const response = await fetch(`${API_URL}/clients?username=${username}`);
        if (!response.ok) {
        ...
        {filteredClients.length > 0 ? (
          filteredClients.map((client) => (
            <a href="#" key={client.id}>
             ...
             <div key={client.id} className="relative px-7 border border-transparont bg-gray-100 
             shadow-[1px_2px_10px_rgba(0,0,0,0.20)] 
          rounded-lg shadow-sm flex flex-col justify-center items-center  text-white">
            
            
            <div className="absolute top-0 right-0 inline-flex divide-x divide-gray-400 overflow-hidden rounded 
           shadow-sm m-2">
                   <button type="button" className="px-1 py-1.5 bg-gray-300 text-sm font-medium text-gray-70
                    hover:bg-gray-500 hover:text-gray-900 focus:relative" aria-label="view">
                    <img src="/edit-svgrepo-com.svg" alt="edit client"  className="size-2 color-white" ></img>
                       
                  </button>

                  <button type="button" className="px-1 py-1.5 bg-gray-300 text-sm font-medium  text-gray-70
                  transition-colors hover:bg-gray-500 hover:text-gray-900 focus:relative " aria-label="View" >
                         <img src="/delete-svgrepo-com.svg" alt="edit client"  className="size-2 color-white" ></img>
                  </button>
              </div>
           
              <img className="w-13 h-13 scale-75 rounded-full" src={client.imageUrl} alt={client.name} />
...
<div className="col-start-4 row-start-auto flex justify-center items-end h-30">
  <button className="flex items-center gap-1 bg-blue-600 text-white font-semibold px-4 py-4 mb-5 rounded-xl shadow-md hover:bg-blue-700 transition-all duration-200 active:scale-95 text-sm sm:text-base">
