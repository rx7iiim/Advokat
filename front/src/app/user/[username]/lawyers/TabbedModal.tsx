...
import Task from "../home/taskInterface";
import ClientModal from "../clients/createClient";
dotenv.config();
...
export default function TabbedModal({
  ...
  const fetchClients = async (username: string) => {
    try {
      const response = await fetch(
        `${API_URL}/clients/clients?username=${username}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDeleteTask = async (taskId: number) => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/task/${taskId}?username=${username}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleAddTask = async () => {
    const input = document.getElementsByName("task")
    const newTask: Task = {
      id: Date.now(),
    ...
    try {
      const response = await fetch(`${API_URL}/task?username=${username}`, {
        ...
        body: JSON.stringify({
          username,
          description: newTaskDescription,
          lawyerUsername: selectedLawyer?.lawyerUsername,
          completed: false,
        }),
      });
        ...
        <div className="p-6 max-h-[400px] overflow-y-auto">
          ...
          {activeTab === "tasks" && (
            <input
              ...
              className="w-1/2 h-8 p-2 border-black border-2"
              onChange={(e) => setNewTaskDescription(e.target.value)}
              placeholder="write the task you want to add here ..."
            />
          )}
          {activeTab === "tasks" && (
            <div>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-medium">
                  Tasks for {selectedLawyer.lawyerUsername}
                </h3>
                <button
          ...
          {activeTab === "clients" && (
            <div>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-medium">
                  Clients for {selectedLawyer.lawyerUsername}
                </h3>
                <button
          ...
          {activeTab === "schedule" && (
            <div>
              <h3 className="text-lg font-medium mb-4">
                Schedule for {selectedLawyer.lawyerUsername}
              </h3>
              <button
